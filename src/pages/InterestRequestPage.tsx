...
export const InterestRequestPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (![form.buyerName, form.cpfCnpj, form.buyerEmail, form.buyerWhatsapp, form.buyerCityState, form.purpose, form.message].every(value => value.trim())) {
      setError('Preencha todos os campos obrigatórios assinalados com asterisco (*).'); return;
    }
    if (!/^\S+@\S+\.\S+$/.test(form.buyerEmail.trim())) {
      setError('Informe um endereço de e-mail de contato válido (ex.: seu.nome@email.com).'); return;
    }
    const documentDigits = form.cpfCnpj.replace(/\D/g, '');
    if (![11, 14].includes(documentDigits.length)) { setError('Informe um CPF (11 dígitos) ou CNPJ (14 dígitos) com quantidade válida de números.'); return; }
    const phoneDigits = form.buyerWhatsapp.replace(/\D/g, '');
    if (phoneDigits.length < 10 || phoneDigits.length > 13) { setError('Informe um WhatsApp válido com DDD (ex: 11 99999-9999).'); return; }
    if (form.message.trim().length < 20) { setError('A mensagem para o compositor deve conter pelo menos 20 caracteres descrevendo seu projeto.'); return; }
    ...
    try {
      ...
      setError(err instanceof Error && err.message
        ? err.message
        : 'Falha ao registrar a solicitação. Verifique sua conexão e tente novamente.');
    } finally {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col"><Navbar /><main className="flex-1 px-4 py-10 sm:py-14"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
      <section className="rounded-3xl border border-slate-800 bg-slate-900 p-5 shadow-2xl sm:p-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-amber-400">Solicitação de liberação de gravação</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-white">Tenho interesse em gravar esta obra</h1><p className="mt-2 text-sm leading-relaxed text-slate-400">Informe seus dados e os detalhes do projeto. O envio não gera cobrança nem autorização automática.</p>
        <ol aria-label="Etapas da solicitação" className="mt-5 grid grid-cols-3 gap-2 text-center text-[11px] font-semibold text-slate-400"><li className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-2 py-2 text-amber-300">1. Seus dados</li><li className="rounded-xl border border-slate-800 bg-slate-950 px-2 py-2">2. Projeto</li><li className="rounded-xl border border-slate-800 bg-slate-950 px-2 py-2">3. Confirmação</li></ol>
...
const Field = ({ label, helper, value, onChange, placeholder, autoComplete, required = false, type = 'text' }: { label: string; helper?: string; value: string; onChange: (value: string) => void; placeholder: string; autoComplete: string; required?: boolean; type?: string }) => {
  const helperId = React.useId();
  return <label className="block text-xs font-semibold text-slate-300">{label}{required ? ' *' : ''}<input required={required} type={type} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} autoComplete={autoComplete} aria-describedby={helper ? helperId : undefined} maxLength={160} className="mt-1.5 w-full rounded-xl border border-slate-800 bg-slate-950 px-3.5 py-3 text-sm text-white focus:border-amber-500 focus:outline-none" />{helper && <span id={helperId} className="mt-1.5 block text-[11px] font-normal leading-relaxed text-slate-500">{helper}</span>}</label>;
};

...
export async function insertSong(userId:string,s:Song){if(!supabase)return;const {error}=await supabase.from('songs').insert({id:s.id,composer_id:userId,...dbSong(s),date_registered:s.dateRegistered});if(error)throw error;}
...
const getVisitorId=()=>{const key='mc_visitor_id';let value=localStorage.getItem(key);if(!value){value=crypto.randomUUID();localStorage.setItem(key,value);}return value;};
...
export async function issueReleaseRequest(requestId:string,data:Pick<ReleaseDocument,'releaseType'|'additionalConditions'|'digitalSignature'>):Promise<ReleaseDocument>{if(!supabase)throw new Error('Supabase não configurado.');const{data:release,error}=await supabase.rpc('issue_release',{p_request_id:requestId,p_release_type:data.releaseType,p_additional_conditions:data.additionalConditions,p_digital_signature:data.digitalSignature});if(error)throw error;return release as ReleaseDocument;}
...
export async function validateReleaseDocument(documentCode:string):Promise<PublicReleaseValidation|null>{if(!supabase)throw new Error('Supabase não configurado.');const{data,error}=await supabase.rpc('validate_release_document',{p_document_code:documentCode});if(error)throw error;return data as PublicReleaseValidation|null;}

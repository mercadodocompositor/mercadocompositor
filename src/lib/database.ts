...
export async function createInterest(songId:string,data:any){if(!supabase)return;const {error}=await supabase.rpc('create_interest_request',{p_song_id:songId,p_data:data});if(error)throw error;}

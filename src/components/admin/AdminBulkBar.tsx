...
export const AdminBulkBar: React.FC<AdminBulkBarProps> = ({
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[900] w-[95%] max-w-2xl animate-slideUp">
      <div className="bg-slate-900/95 backdrop-blur-md border border-amber-500/30 rounded-2xl p-3 sm:p-4 shadow-2xl shadow-black/80 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
          <div className="flex items-center gap-2">
            <span className="w-7 h-7 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-400 font-bold text-xs flex items-center justify-center font-mono">
              {selectedCount}
            </span>
          ...
          <button
            ...
            className="text-[11px] text-slate-400 hover:text-white flex items-center gap-1 hover:underline transition px-2 py-1 rounded-lg"
          >
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end w-full sm:w-auto">
          {actions.map(action => (
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs flex items-center gap-1.5 transition shadow-sm disabled:opacity-50 disabled:cursor-not-allowed ${getVariantStyles(action.variant)}`}
            >
              {action.icon}

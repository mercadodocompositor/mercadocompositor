-- ==============================================================================
-- Correção: conta excluída continuava ligada ao login com Google
-- Execute DEPOIS de fix_banned_until_infinity_2026_09_24.sql.
--
-- A exclusão trocava o e-mail e banía o usuário, mas deixava auth.identities.
-- Quem entrava de novo com o mesmo Google caía na conta excluída e via "User is
-- banned", sem conseguir criar conta nova. O vínculo também guardava nome e
-- e-mail. Agora a exclusão apaga vínculos, fatores de 2 etapas, sessões e
-- tokens; as contas já excluídas são limpas abaixo.
-- ==============================================================================
...
-- Contas já excluídas (e-mail trocado para removido-…@invalido.local).
delete from auth.identities where user_id in (select id from auth.users where email like 'removido-%@invalido.local');
delete from auth.mfa_factors where user_id in (select id from auth.users where email like 'removido-%@invalido.local');
delete from auth.sessions where user_id in (select id from auth.users where email like 'removido-%@invalido.local');
delete from auth.refresh_tokens where user_id in (select id::text from auth.users where email like 'removido-%@invalido.local');
